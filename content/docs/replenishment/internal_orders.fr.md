...
Une liste de toutes les commandes internes existants s'affiche :

![Internal Order: list](/docs/replenishment/images/intord_list.png)

Vous pouvez utiliser les filtres pour trier la liste par nom ou par statut des commandes internes :

![Internal Order: filter](/docs/replenishment/images/intord_filter.png)

Vous pouvez trier la liste en utilisant les en-têtes de colonne :
...
3. Sélectionnez un fournisseur en cliquant sur son nom. Vous serez redirigé vers une fenêtre comme celle-ci :
...
Sélectionnez d'abord l'article que vous souhaitez ajouter à votre commande. Vous pouvez également saisir tout ou une partie du nom (ou du code) d'un article pour le rechercher de manière spécifique.
...
1. Cliquez sur le bouton `Ajouter à partir de la liste principale`
...
Cliquez sur une ligne de commande pour modifier la **Quantité demandée** ou tapez sur le bouton `Quantités suggérées pour l'utilisation`
...
Si vous cliquez sur le bouton `Quantités suggérées pour l'utilisation` dans l'en-tête de la demande d'achat, mSupply copiera automatiquement les valeurs de l'en-tête de la demande d'achat. **Quantité suggérée** dans la colonne **Quantité demandée**.
...
1. Cliquez sur `Confirmer l'envoi` (en bas à droite)
...
3. Le statut de votre commande est maintenant `ENVOI` et la commande n'est plus modifiable
<div class="warning">
...
De plus, lors de la consultation d'une commande interne spécifique, des colonnes indiquent la quantité approuvée, le nombre d'emballages approuvés et un commentaire saisi par l'approbateur, s'il y en a un :
