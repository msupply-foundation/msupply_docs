...
L'une des tâches les plus importantes — mais aussi les plus simples — dans mSupply est de vérifier la quantité de stock disponible. Lorsque vous créez une Expédition Sortante et ajoutez un article, mSupply vous indique si vous avez suffisamment de stock. Mais à bien d'autres moments, vous pouvez avoir besoin de vérifier rapidement, et c'est très simple à faire.

## Consulter le stock

Dans le panneau de navigation, allez dans `Inventaire` et appuyez sur le sous-menu `Stock` :
...
| **Taille de paquet**  | La taille de paquet de l'article                                                                                                                                                               |
| **Qté paquets**       | Quantité totale de stock dans votre dépôt, en paquets                                                                                                                                          |
| **SED**               | Quantité totale de stock dans votre dépôt, en unités                                                                                                                                           |
| **SED disponible**    | Stock en dépôt disponible (non alloué à la distribution), en unités                                                                                                                            |
| **Coût paquet**       | Prix de coût par paquet                                                                                                                                                                        |
| **Total**             | Valeur totale du SED (`Qté paquets x Coût paquet`)                                                                                                                                             |
...
- Vous pouvez émettre ce stock dans une expédition sortante ou une prescription
...
Vous ne pouvez pas non plus commander davantage de cet article — il ne peut pas être ajouté à une expédition entrante manuelle ni à une commande interne.

## Créer une nouvelle ligne de stock

<div class="note">
Il est rare que vous ayez besoin de créer une nouvelle ligne de stock de cette façon. Votre stock devrait être introduit via des Expéditions Entrantes, ou ajusté via un Inventaire.
</div>

Pour créer une nouvelle ligne de stock, cliquez sur le bouton `Nouveau Stock` en haut à droite de votre écran.
...
Les variantes d'articles incluent des informations d'emballage — si votre nouvelle ligne de stock a une taille de paquet correspondante, le champ `Volume par paquet` sera automatiquement rempli.
...
Spécifiez le volume d'un seul paquet en mètres cubes. Cela est utilisé pour calculer l'espace de stockage disponible lorsque le stock est stocké dans un emplacement. Notez que vous devrez également enregistrer le volume de l'emplacement pour voir la [capacité restante](/docs/inventory/locations/#assigning-locations-to-stock) de l'emplacement.

Le champ `Volume total` ci-dessous affiche la valeur calculée du volume utilisé par cette ligne de stock, qui correspond au volume par paquet multiplié par le nombre de paquets (`Qté paquets`).
...
Notez que vous ne pouvez pas modifier `Qté paquets` ou `Taille de paquet` depuis cette vue. Vous pouvez le faire via les fonctions [Reconditionnement](#reconditionner-le-stock) et [Ajustements](#ajuster-le-niveau-de-stock).

`Case à cocher En attente` : Pour mettre une ligne de stock en attente, cochez la case En attente. Une fois une ligne de stock mise en attente, elle ne peut plus être émise. Elle sera toujours incluse dans le calcul du stock disponible, mais les lignes de stock en attente ne sont pas disponibles pour l'émission dans les expéditions sortantes ni les prescriptions.
...
Une fois le code-barres mis à jour, ce code est associé à l'article pour cette taille de paquet particulière. Cet article sera maintenant automatiquement détecté lors de l'ajout d'articles à une Expédition Sortante via un scanner de codes-barres.

Les codes-barres mis à jour de cette façon seront également synchronisés avec d'autres dépôts, ce qui signifie que les codes que vous scannez ici permettront à d'autres dépôts d'ajouter automatiquement ces articles à des Expéditions Sortantes via un scanner de codes-barres.
...
Pour mettre à jour le statut VVM de ce stock, cliquez sur le bouton `Ajouter une entrée de statut`. Une fenêtre s'ouvre vous permettant de choisir le niveau de statut et d'entrer optionnellement un commentaire.
...
La fonctionnalité de reconditionnement permet de fractionner le stock en tailles de paquets plus petites, de le regrouper en tailles plus grandes ou de déplacer tout ou partie d'une ligne de stock vers un nouvel emplacement.

Dans le coin supérieur droit de la page de détails de la ligne de stock, cliquez sur le bouton `Reconditionner`.
...
Depuis ici, vous pouvez saisir le nombre de paquets que vous souhaitez reconditionner, jusqu'à un maximum du nombre actuel de paquets en stock (représenté par le nombre à côté de `Paquets disponibles`) — dans ce cas, 5 779 paquets sont disponibles.

Vous pouvez ensuite saisir une nouvelle taille de paquet, et optionnellement un emplacement pour le nouveau stock. Le `Nouveau nombre de paquets` est calculé automatiquement. Cliquez sur `Enregistrer` pour sauvegarder les modifications. Cliquer sur `Imprimer` vous permettra d'imprimer les détails de cette opération de reconditionnement.
...
Une nouvelle fenêtre s'ouvrira, où vous pouvez indiquer si vous souhaitez augmenter ou diminuer la quantité de paquets, et de combien.

![Formulaire d'ajustement du stock](images/stock_adjust_form.png)

Si vous avez configuré des [raisons d'ajustement d'inventaire](https://docs.msupply.org.nz/preferences:options?s[]=reasons) sur votre serveur central, vous devez également saisir une raison lors de l'ajustement de la quantité de paquets.

Si c'est le cas, le champ de raison sera activé comme ci-dessous :

![Nouvelle ligne de stock : saisir une raison](images/stock_new_reason.png)

Lorsque vous êtes prêt à ajuster le niveau de stock, cliquez sur le bouton `OK`. Vous verrez alors votre quantité de paquets mise à jour dans l'[onglet Détails](#onglet-détails), et pourrez consulter l'ajustement dans l'[onglet Grand Livre](#onglet-grand-livre).
