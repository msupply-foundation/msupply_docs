...
Demographics management is only available on the [Open mSupply Central Server](/docs/v3/getting-started/central-server).
...
Demographics are only visible on the [Open mSupply Central Server](/docs/v3/getting-started/central-server) and requires the [`mobile: Uses vaccine module`](https://docs.msupply.org.nz/cold_chain_equipment:mobile?s[]=vaccine#enable_the_vaccine_module_for_the_mobile_store) store preference.
