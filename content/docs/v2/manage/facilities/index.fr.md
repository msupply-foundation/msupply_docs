...
La liste des dépôts est disponible uniquement sur le [Serveur Central Open mSupply](/docs/v3/getting-started/central-server). C'est ici que vous pouvez consulter tous les dépôts et gérer leurs propriétés.
...
| **Utiliser l'interface mobile simplifiée**                                                                 | Réduit le nombre de champs et d'éléments pour les tablettes — voir la page [interface tablette simplifiée](/docs/v3/settings/simplified-ui). Nécessite une préférence de dépôt héritée.                  |
...
<div class='note'>
La plupart des préférences de dépôt sont encore gérées via le serveur central mSupply hérité (voir la <a href="/docs/v3/settings/configuration/#store-preferences">liste des préférences</a>). Seules les nouvelles préférences de dépôt pour Open mSupply sont configurées sur le serveur central Open mSupply pour l'instant. Toutes les préférences de dépôt seront migrées vers Open mSupply dans une version future.
</div>
...
Les dépôts ne sont visibles que sur le [Serveur Central Open mSupply](/docs/v3/getting-started/central-server).
