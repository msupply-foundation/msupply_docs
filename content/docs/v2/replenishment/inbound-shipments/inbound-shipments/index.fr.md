...
<div class="tip">Voir la <a href="/docs/v3/introduction/faq/#is-there-support-for-my-currency">question sur la prise en charge des devises</a> pour la liste des codes pris en charge</div>
...
L'onglet Quantités affichera des colonnes supplémentaires lorsque certaines préférences sont activées dans [Gérer > Dépôts](/docs/v3/manage/facilities/#editing-store-preferences).
...
2. Si l'article a une `marge` définie, cette marge est ajoutée au prix de coût. La priorité entre marge article et marge fournisseur est déterminée par la [Préférence globale](/docs/v3/manage/global-preferences/) `La marge article prime sur la marge fournisseur`.
...
- **Fabricant** : si des [Variantes d'Articles](/docs/v3/catalogue/items/#item-variants) sont configurées, vous pouvez voir et modifier le fabricant.
...
Pour commencer, activez la [Préférence globale](/docs/v3/manage/global-preferences/) `Permettre le suivi du stock par donateur`. Puis configurez des donateurs dans le serveur central mSupply.
...
Si la [préférence du dépôt](/docs/v3/settings/configuration/#store-preferences) `Peut lier manuellement une réquisition à une facture fournisseur` est activée, il est possible de lier une expédition entrante créée manuellement à une commande interne.
