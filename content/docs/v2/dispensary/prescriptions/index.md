...
<li><code>Dispensing</code> - the pharmacy's record of stock issued to a patient. In earlier versions this screen was called <code>Prescriptions</code>; it is described on the <a href="/docs/v3/dispensary/dispensing/">Dispensing</a> page.</li>
</ul>
</div>
...
The permission is granted in mSupply, on the `omSupply Permissions` tab: **View/edit prescriptions**. See the [Permissions](/docs/v3/settings/permissions/) page for the full list of available permissions.
...
| :------------ | :------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Patient**   | Yes      | Start typing a name or code and select the patient from the list. If no match is found you can create a new patient here, exactly as when creating a [dispensing record](/docs/v3/dispensary/dispensing/#select-a-patient)                      |
...
- A mistake noticed after the prescription is `Ready to dispense` is handled on the dispensing record, by editing it before it is verified or cancelling it after (see [Cancelling a Prescription](/docs/v3/dispensary/dispensing/#cancelling-a-prescription)).
...
The dispenser opens the record in `Dispensary` > `Dispensing`. It works like any other dispensing record (see [Dispensing](/docs/v3/dispensary/dispensing/)), with a few differences because it came from a prescription:
...
Prescriptions support [custom fields](/docs/v3/manage/custom-fields/) under their own `Prescription requests` record type. Unlike other record types, four fields are provided by default on every system with nothing to configure in mSupply:
...
Users with the Prescription request query permission, in a dispensary store, see a `Prescriptions` panel on the [home page](/docs/v3/getting-started/home/), with a `Prescription request` section showing:
