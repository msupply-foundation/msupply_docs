+++
title = "Dispensing"
description = "Dispensing stock to patients."
date = 2022-03-19
...
Dispensing records are used to issue medicine to patients. A dispensing record takes stock out of your store and records which patient received it, the clinician, the directions and any payment.

<div class="note">
<b>Naming.</b> Before Open mSupply v3.2 this screen was called <code>Prescriptions</code>, and dispensing records are still referred to as <i>prescriptions</i> throughout this page and in some messages in the app. From v3.2 the <code>Prescriptions</code> entry in the <code>Dispensary</code> menu is a separate screen where prescribers record what a patient should receive <i>before</i> it is dispensed - see <a href="/docs/v3/dispensary/prescriptions/">Prescriptions</a>.
</div>

## Viewing Dispensing Records

To use dispensing, ensure that your store is set to `Dispensary` mode (see the [virtual stores](https://docs.msupply.org.nz/other_stuff:virtual_stores#store_type) documentation for how to do this).

### Open the Dispensing Menu

Choose `Dispensary` > `Dispensing` in the navigation panel.

![Dispensary navigation menu showing the Dispensing option](images/goto_prescriptions.png)

You will be presented with a list of dispensing records (if you don't see any, you are probably just starting!).

From this screen you can:

- View a list of dispensing records
- Create a new dispensing record
- Export the list to a `.csv` file

### Dispensing Record List

1. The list of dispensing records is divided into 5 columns:
...
<div class="note">
You can only delete Prescriptions with a status <code>NEW</code> or <code>PICKED</code>. Verified prescriptions can be cancelled, but not deleted. Prescriptions which were generated from a <a href="/docs/v3/dispensary/prescriptions/">prescription</a> cannot be deleted at all - see <a href="#dispensing-a-prescription-request">Dispensing a Prescription Request</a>.
</div>
...
1. Go to `Dispensary` > `Dispensing`

2. Press the `New dispensing record` button, in the top right corner

![New dispensing record button in the top right corner of the dispensing list](images/click_new_prescription.png)

3. The `Create dispensing record` window opens, in which you can select a patient and other details for the prescription

![Create dispensing record window with patient search field and optional prescription details](images/prescription_new_prescription_name.png)

#### Select a Patient

In the `Create dispensing record` window, you will be presented with a form to search for available patients. The patients list will load when you start typing - once you see the correct patient on your screen you can select it (you do not need the enter the full name or code)
...
Insurance details can be viewed in the Insurance tab if you have setup [Insurance Providers](/docs/v3/dispensary/patients/#insurance) in mSupply</a>
...
Tap on the `Close` button to quit the dispensing record and return to the Dispensing list.
...
<div class="note">There is a special case when issuing a vaccine item and the <a href="/docs/v3/manage/facilities/#store-preferences">Sort available batches by VVM status then expiry</a> preference is enabled.<br/><br/>
...
Vaccine items can be dispensed in doses rather than units, if the [Manage vaccines in doses](/docs/v3/manage/facilities/#store-preferences) store preference is enabled.

The following quantity columns will be displayed instead when issuing a vaccine item:

| Column               | Description                                                                                                                                                                  |
| :------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **VVM Status**       | If the [Manage VVM status for stock](/docs/v3/manage/facilities/#store-preferences) store preference is enabled, a column displaying the VVM status of this stock line is shown |
...
<div class="tip">
If some stock you are expecting to see doesn't show as available, use the <a href="/docs/v3/inventory/stock-view/#ledger-tab">Ledger</a> in Inventory -> Stock to see when that stock was received.
</div>
...
## Dispensing a Prescription Request

When a prescriber marks a [prescription](/docs/v3/dispensary/prescriptions/) as `Ready to dispense`, Open mSupply creates a dispensing record for it automatically. It appears in the `Dispensing` list with a status of `NEW`, and is processed in the same way as any other record on this page, with the following differences:

- The **Patient**, **Clinician** and **Diagnosis** are read-only. They were set by the prescriber.
- Each line is already present, with the **Prescribed quantity** and **Directions** filled in but no stock allocated. The prescribed quantity is read-only and is used as the target when you open the line and stock is automatically allocated. You can still adjust which batches are used and the quantity issued.
- The side panel has a `Prescription` link in the `Related documents` section which opens the original prescription (visible only to users who have the Prescription request query permission).
- The record cannot be deleted, even while `NEW`. If it should not be dispensed, verify it and then [cancel](#cancelling-a-prescription) it.

When you confirm the record as `Verified`, the original prescription request is automatically updated to `Dispensed`.
...
To begin printing labels, first ensure that you have configured a printer in [Settings](/docs/v3/settings/devices/).
