...
<div class="note">
If the <a href="/docs/v3/manage/facilities/#store-preferences">Manage VVM status for stock</a> store preference is enabled, a VVM status filter will also be available.
</div>
...
When counting a blind stocktake, the snapshot and difference values are hidden. The expected quantities and the calculated difference are still recorded behind the scenes, and become visible again once the stocktake is finalised, so you can review the variance and it is still captured in the [stocktake variance report](/docs/v3/getting-started/reports/).

To enable the blind stocktake feature, enable the `Enable blind stocktake` store [preference](/docs/v3/manage/facilities/#available-preferences).

#### Vaccines

If you have the [Manage vaccines in doses](/docs/v3/manage/facilities/#store-preferences) store preference enabled you will see columns for `Doses per unit` and `Doses counted`. For stocktake lines of vaccine items, the number of doses per unit (e.g. `Vial`) is shown in this column. The `Difference` column will also show the difference in doses as well as packs:
...
If [Item Variants](/docs/v3/catalogue/items/#item-variants) are configured, you can select a variant using the Item Variant selection panel, which displays variants as clickable cards showing the variant name, manufacturer, and VVM type (for vaccines). Selecting a variant will automatically set the manufacturer. You can also choose `Manual input` to enter a manufacturer manually.
...
<div class="note">You are not able to edit the pack size of stocktake rows which are linked to an existing stock line. If you wish to repack, follow the instructions in <a href="/docs/v3/inventory/stock-view/#repacking-stock">Repack</a>.</div>
...
You can update lots of information about the stock line as you do your stocktake, such as the batch number, expiry date, and location. If you are using the <a href="/docs/v3/manage/facilities/#store-preferences">Manage VVM status for stock</a> store preference, you will also be able to update the VVM status of vaccine items.
...
If you use [campaigns](/docs/v3/manage/campaigns/), a `Campaign` column and filter are available in the stocktake view so you can see and narrow down which stock belongs to which campaign.
