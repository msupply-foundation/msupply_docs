...
(see [Manage > Demographics](/docs/v3/manage/demographics) to configure an indicator)
...
(see [Programs > Immunizations](/docs/v3/programs/immunisations/))
...
see [Manage > Store properties](/docs/v3/manage/facilities/#editing-your-store-properties) for details.
...
**Note 2**: The `Forecast demand` is calculated for the [`Vaccine course`](/docs/v3/programs/immunisations/#vaccine-courses), not the [`Vaccine item`](/docs/v3/catalogue/items/). This means that the `Target stock` and `Suggested quantity` values also apply to the `Vaccine course` and not to the `Vaccine item`.
