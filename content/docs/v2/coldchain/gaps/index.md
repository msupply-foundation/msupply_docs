...
CCE can be registered for every facility in the country by following the documentation for the [CCEI module](/docs/v3/coldchain/equipment/).
...
1. Manually - each facility user submits a [functional status report](/docs/v3/coldchain/equipment/#update-status) for each of their CCE
2. Automatically - each CCE directly reports its own functional status (\*_future feature_)

As of version 2.7, only the manual option is available.

Therefore, to obtain an accurate `CCE functioning capacity` it is **vitally important to implement a robust SOP** for facilities so that they regularly report the functional status of their CCE by submitting a [functional status report](/docs/v3/coldchain/equipment/#update-status) (for example, on a weekly basis)
...
The volume required to store each type of vaccine can be recorded in the [Item Catalogue](/docs/v3/catalogue/items/#packaging-1).
...
![Item variant detail for BCG vaccine showing packaging levels with volume per unit in litres](images/item_variant_volume.png)
...
Each facility can submit a snapshot of their own `Population served` data in the [Store properties](/docs/v3/manage/facilities/#editing-your-store-properties) section.
...
The `Population indicators` are set at the central level in the [Demographics](/docs/v3/manage/demographics/#adding-a-new-indicator) section.
...
The `Population growth` may be specified for the next 5 years in the [Demographics](/docs/v3/manage/demographics/#yearly-growth-percentage) section.
...
The different vaccine courses that form part of the national immunization program are specified at the central level in the [Programs > Immunizations](/docs/v3/programs/immunisations/#vaccine-courses) section.
